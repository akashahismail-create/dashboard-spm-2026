...
with col_butang2:
    ...
    if st.button("📊 PENGURUSAN SPM", key="btn_pengurusan", use_container_width=True, type="secondary"):
        st.markdown(f'<meta http-equiv="refresh" content="0; url={LINK_PENGURUSAN}">', unsafe_allow_html=True)
        st.rerun()

# ===== POPUP PASSWORD UNTUK BUTANG CALON =====
    ...
    with st.form("form_password"):
        ...
        if submit:
            if pw == "spmB":
                ...
                time.sleep(1)
                st.markdown(f'<meta http-equiv="refresh" content="0; url={LINK_CALON}">', unsafe_allow_html=True)
                st.rerun()
            else:
